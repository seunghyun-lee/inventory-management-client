...
function ItemManagement() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                const response = await axios.put(`${API_BASE_URL}/api/items/${newItem.id}`, newItem);
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-100">
                        <tr className="text-xs">
                            <th className="w-[22%] px-3 py-2 text-left font-semibold border-b">물품명</th>
                            <th className="w-[18%] px-3 py-2 text-left font-semibold border-b">뒷부호</th>
                        ...
                        <tbody className="text-xs bg-white">
                            {filteredItems.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50">
                                    <td 
                                        className="w-[22%] px-3 py-2 text-left border-b cursor-pointer hover:text-blue-600"
                                        onClick={() => handleItemClick(item)}
                                    >
                                        {item.item_name}
                                    </td>
                                    <td className="w-[18%] px-3 py-2 text-left border-b">{item.item_subname}</td>
